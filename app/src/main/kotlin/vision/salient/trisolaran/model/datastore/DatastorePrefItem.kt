...
import org.jdc.template.model.datastore.DatastorePrefCustomItem
import org.jdc.template.model.datastore.DatastorePrefEnumItem
import org.jdc.template.model.datastore.DatastorePrefItem
import org.jdc.template.model.datastore.DatastorePrefValueClass
import org.jdc.template.model.datastore.DefaultDatastorePrefItem
import vision.salient.trisolaran.util.ext.mapDistinct
...
class DefaultDatastorePrefItem<T: Any?>(
    ...
    override val flow: Flow<T> = vision.salient.trisolaran.util.ext.mapDistinct { preferences ->
...
class DatastorePrefValueClass<PREF_T, VALUE_CLASS_T: Any?>(
    ...
    override val flow: Flow<VALUE_CLASS_T> = vision.salient.trisolaran.util.ext.mapDistinct { preferences ->
        preferences[preferenceKey]?.let { prefValueToValueClass(it) } ?: defaultValue
    }
...
class DatastorePrefEnumItem<T : Enum<T>>(
    ...
    override val flow: Flow<T> = vision.salient.trisolaran.util.ext.mapDistinct { preferences -> preferenceKeyToEnum(preferences[preferenceKey]) }
    override suspend fun setValue(value: T) {
