package vision.salient.trisolaran.work
...
@Inject constructor(
    ...
    fun scheduleSimpleWork(text: String) {
        val inputData = org.jdc.template.work.SimpleWorker.createInputData(text)
    ...
    fun scheduleSync(now: Boolean = false) {
        ...
        workManager.beginUniqueWork(org.jdc.template.work.SyncWorker.UNIQUE_WORK_NAME, ExistingWorkPolicy.REPLACE, workRequest)
            .enqueue()
    }

    fun scheduleRemoteConfigUpdate() {
        val workRequest = createNetworkPeriodicWorkRequest<RemoteConfigSyncWorker>(5, TimeUnit.DAYS)
        workManager.enqueueUniquePeriodicWork(org.jdc.template.work.RemoteConfigSyncWorker.UNIQUE_PERIODIC_WORK_NAME, ExistingPeriodicWorkPolicy.KEEP, workRequest)
    }
